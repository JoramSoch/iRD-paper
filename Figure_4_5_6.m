...
if ~exist(strcat('Figure_4_5_6_',DA_meas,'.mat'), 'file')
    ...
    % save results
    save(strcat('Figure_4_5_6_',DA_meas,'.mat'), 'subj_ids', 'subj_grp', 'DAPs_beh', 'DAPs_des');
    
% load, if file does exist
else
    
    % load results
    load(strcat('Figure_4_5_6_',DA_meas,'.mat'));
...
for i = 1:2
    ...
    for k = 1:numel(ana_ids)
        ...
        set(h, 'LineWidth', 1);
      % set(gca, 'TickLabelInterpreter', 'latex', 'XTickLabelRotation', 0);
        set(gca, 'XTick', [1:5], 'XTickLabel', Z_labs(3:6), 'FontSize', 14);
        xlabel('decoding algorithm', 'FontSize', 16);
        if strncmp(ana_ids{k},'1',1) || strncmp(ana_ids{k},'2',1)
